...
def costFunction(theta, X, y):
    ...
    J = -(np.dot(y,np.log(hypothesis))+np.dot((1-y),np.log(1-hypothesis)))/m
...
def costFunctionReg(theta, X, y, lambda_):
    ...
    # ===================== YOUR CODE HERE ======================
    hypothesis=sigmoid(np.dot(X, theta))   
    J=-(np.dot(y, np.log(hypothesis))+np.dot((1-y), np.log(1-hypothesis))-1/2*lambda_*np.sum(np.square(theta[1:])))/m
    thetaa=np.insert(theta[1:], 0, 0)
    grad=(np.dot(X.T, (hypothesis-y))+lambda_*thetaa)/m 
